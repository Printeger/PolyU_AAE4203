...
obss = gt.Gobs(fullfile('E:\TA\rtklib_matlab\data\gnss_log_2024_09_24_17_44_17.24o'));
navs = gt.Gnav(fullfile('E:\TA\rtklib_matlab\data\*.24*'));
...
num_epochs = size(obss.L1.P, 1); % 获取总历元数

%% Initialize storage for receiver positions
Receiver_positions = zeros(num_epochs, 3); % 存储每个历元的接收机位置

%% Initialize cell arrays to store data for each epoch
pseudoranges_meas_cell = cell(num_epochs, 1); % 伪距观测值
satellite_positions_cell = cell(num_epochs, 1); % 卫星位置
satellite_clock_bias_cell = cell(num_epochs, 1); % 卫星钟差
ionospheric_delay_cell = cell(num_epochs, 1); % 电离层延迟
tropospheric_delay_cell = cell(num_epochs, 1); % 对流层延迟
...
for epoch = 1:num_epochs
    ...
    if epoch > 1
        Receiver_pos = Receiver_positions(epoch - 1, :)'; % 使用上一历元的位置作为初始值
    else
        Receiver_pos = [0; 0; 0]; % 第一个历元，初始位置为零
    end
    receiver_clock_err = [0; 0; 0; 0]; % GPS, BEIDOU, GALILEO, GLONASS

    %% Build design matrix and geometric distance
    num_sats = length(p_l1);
    H = zeros(num_sats, 3 + 4); % 3个位置未知数 + 4个时钟偏差未知数
    ...
    max_iterations = 10; % 设置最大迭代次数，防止死循环
    for iter = 1:max_iterations
        ...
        W = diag(1./(ion_error_l1.^2 + tropo_error.^2 + 1)); % 加上一个常数，防止除零
